...
public:
    explicit ConnectionSignalTracker(const Callback callback = nullptr)
        : _callback(callback) {}
    RssiType rssi() const {
        return _rssiLast;
    }
...
public:
    explicit NetworkTimeFetcher(const String& useragent, const String& server)
        : _useragent(useragent), _server(server) {}
    time_t fetch() {
        HTTPClient client;
        String header = "";
        time_t time = 0;
...
public:
    ...
    long size() const {
        if (_mode == MODE_CLOSED)
            if (_size < 0) const_cast<SPIFFSFile*>(this)->_ssize();    // zero if not exists
    ...
    size_t write(const T& data) {
        if (_mode == MODE_READING) _close();
        if (_mode == MODE_CLOSED)
            if (_erase()) _open(MODE_WRITING);    // will set ERROR
        if (_mode == MODE_ERROR) return 0;
        return _append(data);
    }
    template<typename T>
