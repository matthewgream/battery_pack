...
public:
    explicit WebServer(const Config &cfg)
        : config(cfg), _server(config.port) {}
...
public:
    explicit WebSocket(const Config &cfg)
        : Singleton<WebSocket>(this), ConnectionReceiver<WebSocket>::Insertable(&_connectionReceiver), config(cfg), _server(config.port), _socket(config.root),
...
public:
    explicit MQTTPublisher(const Config &cfg)
        : config(cfg), _mqttClient(_wifiClient) {}
