...
public:
    explicit LoggingHandler(const Config& cfg, const String& id, MQTTPublisher* mqttClient = nullptr)
        : Singleton<LoggingHandler>(this), config(cfg), _mqttClient(mqttClient), _mqttTopic(config.mqttTopic + "/logs/" + id) {
        init();
    }
    ~LoggingHandler() {
        term();
...
public:
    UpdateManager(const Config& cfg, const BooleanFunc networkIsAvailable)
        : Alarmable({
            AlarmCondition(ALARM_UPDATE_VERS, [this]() {
                return _available;
            }),
            AlarmCondition(ALARM_UPDATE_LONG, [this]() {
                return istoolong();
            }),
        }),
