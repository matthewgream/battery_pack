...
class BluetoothManager(
    ...
    private fun deviceReconnect() {
        if (reconnectAttempts < MAX_RECONNECT_ATTEMPTS) {
            reconnectAttempts++
            Log.d("Bluetooth", "Attempting to reconnect. Attempt $reconnectAttempts of $MAX_RECONNECT_ATTEMPTS")
            handler.postDelayed({
                if (!isConnected) {
                    scanStart()
                }
            ...
            Log.e("Bluetooth", "Failed to reconnect after $MAX_RECONNECT_ATTEMPTS attempts")
            statusCallback(false)
        }
    }
