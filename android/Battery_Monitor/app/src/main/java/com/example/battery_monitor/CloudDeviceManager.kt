            ...
            connect?.send ()
                ?.whenComplete { connAck: Mqtt5ConnAck, throwable ->
                    if (throwable != null) {
                        ...
                        Log.d("Cloud", "MQTT broker connection success: ${connAck.reasonString.map { it.toString() }.orElse("Success")}")
                        isConnected = true
                        isConnecting = false
                        statusCallback ()
                        identify ()
    ...
    fun subscribe (topic: String) {
        ...
        try {
            ...
            ?.callback { publish: Mqtt5Publish ->
                ...
                Log.d("Cloud", "MQTT broker subscribe '${publish.topic}' message: '$message' [type=${publish.contentType.map { it.toString() }.orElse(null)}, topic=${publish.responseTopic.map { it.toString() }.orElse(null)}]")
            ...
            ?.whenComplete { subAck: Mqtt5SubAck, throwable ->
                if (throwable != null)
                    ...
                    this.topic = topic
                    Log.d("Cloud", "MQTT broker subscribe '$topic' success: ${subAck.reasonCodes.joinToString(", ")}")
                }
            }
        } catch (e: Exception) {
